...
void TestYourApp::checkCalendar()
{
    ...
    {
        QVERIFY2(lbl_calendar->text() == "Введите год",
    ...
    if (rb_tibet->isChecked() or rb_zoroast->isChecked())
    {
        ...
        QVERIFY2(lbl_calendar->text() == expected_calendar,
...
void TestYourApp::checkHoroscope()
{
    ...
    {
        QVERIFY2(lbl_horoscope->text() == "Ввод даты отключён",
    ...
    if (rb_zodiak->isChecked())
    {
        ...
        QString expected_horoscope =
            QString("%1.%2 — ваш знак зодиака %3 по зодиакальному гороскопу")
                .arg(day, 2, 10, QChar('0')).arg(month + 1, 2, 10, QChar('0')).arg(expected_symbol);

        QVERIFY2(lbl_horoscope->text() == expected_horoscope,
    ...
    else
    {
        ...
        QString expected_horoscope =
            QString("%1.%2 — ваше тотемное дерево %3 по гороскопу друидов")
                .arg(day, 2, 10, QChar('0')).arg(month + 1, 2, 10, QChar('0')).arg(expected_tree);
